...
class LLM:
    ...
    def _load_keys(self) -> list[str]:
        keys = []
        key1 = os.getenv("ANTHROPIC_API_KEY_1")
        ...
        if key2:
            keys.append(key2)
        if not keys:
            raise ValueError("No Anthropic API keys found. Set ANTHROPIC_API_KEY_1 in .env")
        return keys
    ...
    def _switch_key(self):
        next_index = self._current_key_index + 1
        if next_index >= len(self._api_keys):
            raise RuntimeError("All Anthropic API keys are rate limited. Try again later.")
        self._current_key_index = next_index
        self._setup_client()
    ...
    def _is_rate_limit_error(self, e: Exception) -> bool:
        if isinstance(e, anthropic.RateLimitError):
            return True
        return "429" in str(e) or "rate" in str(e).lower() or "overloaded" in str(e).lower()
